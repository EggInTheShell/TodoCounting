...
def generate_patches_from(image, label):
    ...
    new_label = np.zeros([new_height, new_width, 5], dtype=np.uint8)
    new_label[:height, :width] = label

    image_patches = patch_split_of(new_image, patch_size)
    label_patches = patch_split_of(new_label, patch_size)
...
def generate_segmentation_patches():
    ...
    label_folder = DATA_DIR + 'segmentation_images/'
    ...
    for item in label_path_list:
        ...
        with open(labelpath, mode='rb') as f:
            label = pickle.load(f)
        image = np.asarray(Image.open(train_folder + str(id) + '.png'))
